    ...
    <form action="{{ route('superadmin.documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="category" name="category" required onchange="toggleSOPFields()">
                ...
                <option value="BAB 1" {{ $document->category == 'BAB 1' ? 'selected' : '' }}>BAB 1 (Pendahuluan)</option>
                <option value="BAB 2" {{ $document->category == 'BAB 2' ? 'selected' : '' }}>BAB 2 (Gambaran Umum)</option>
                <option value="BAB 3" {{ $document->category == 'BAB 3' ? 'selected' : '' }}>BAB 3 (Struktur Organisasi)</option>
                <option value="BAB 4" {{ $document->category == 'BAB 4' ? 'selected' : '' }}>BAB 4 (Pelayanan Manajemen Lalu Lintas Dan Telekomunkasi Penerbangan)</option>
                <option value="BAB 5" {{ $document->category == 'BAB 5' ? 'selected' : '' }}>BAB 5 (Fasilitas Dan Peralatan)</option>
                <option value="BAB 6" {{ $document->category == 'BAB 6' ? 'selected' : '' }}>BAB 6 (Personel)</option>
                <option value="BAB 7" {{ $document->category == 'BAB 7' ? 'selected' : '' }}>BAB 7 (Standar Pelayanan)</option>
                <option value="BAB 8" {{ $document->category == 'BAB 8' ? 'selected' : '' }}>BAB 8 (Sistem Manajemen Keselamatan)</option>
                <option value="BAB 9" {{ $document->category == 'BAB 9' ? 'selected' : '' }}>BAB 9 (Rencana Kontigensi)</option>
                <option value="BAB 10" {{ $document->category == 'BAB 10' ? 'selected' : '' }}>BAB 10 (Dokumen Dan Rekaman)</option>
                <option value="BAB 11" {{ $document->category == 'BAB 11' ? 'selected' : '' }}>BAB 11 (Prinsip Human Faktor)</option>
        ...
        <a href="{{ route('superadmin.documents.index') }}" class="btn btn-warning">Batal</a>
    </form>
</div>
