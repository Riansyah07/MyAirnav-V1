...
<div class="container bg-white p-4 rounded shadow-sm">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('admin.isr.create') }}" class="btn btn-primary">+ Tambah Dokumen</a>
    </div>
            ...
            <tbody>
                @forelse($isrs as $item)
                    <tr id="row-{{ $item->id }}">
                        ...
                        <td>
                            <a href="{{ route('admin.isr.show', $item->id) }}" class="btn btn-sm btn-info">Lihat</a>
                            <a href="{{ route('admin.isr.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>

                            {{-- Form Hapus (jangan nested dalam form lain) --}}
                            <form action="{{ route('admin.isr.destroy', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus dokumen ini?')">
                                @csrf
                                @method('DELETE')
